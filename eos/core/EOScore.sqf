...
_heightLimit=if(count _setup>4)then{_setup select 4}else{false};
...
if isMultiplayer then{
	...
	_actCond="{vehicle _x in thisList && isPlayer _x && !(vehicle _x isKindOf ""Plane"")&&((getPosATL _x)select 2)<100}count playableUnits>0";}else{
	_actCond="{vehicle _x in thisList && isPlayer _x}count playableUnits>0";};}else{
	if(_heightLimit)then{
	_actCond="{vehicle _x in thisList && isPlayer _x && !(vehicle _x isKindOf ""Plane"")&&((getPosATL _x)select 2)<100}count allUnits>0";}else{
...
_eosActv setTriggerTimeout[1,1,1,true];
...
waitUntil{triggerActivated _eosActv};
...
if(isNil "_aGrp")then{_aGrp=[];};
	...
	_units=_eosActv getVariable _cacheGrp;	
	_aSize=[_units,_units];
	_aMin=_aSize select 0;
		if(_dbug)then{player sideChat format["ID:%1,restore - %2",_cacheGrp,_units];};};
			if(_aMin>0)then{
...
if(_dbug)then{player sideChat(format["Spawned House Patrol: %1",_counter]);0=[_mkr,_counter,"House Patrol",getPos(leader _aGroup)]call EOSdbug};};};
...
if(_dbug)then{player sideChat(format["Spawned Patrol: %1",_counter]);0=[_mkr,_counter,"patrol",getPos(leader _bGroup)]call EOSdbug};};};	
...
if(_dbug)then{player sideChat format["Light Vehicle:%1 - r%2",_counter,_cGrps];0=[_mkr,_counter,"Light Veh",(getPos leader(_cGroup select 2))]call EOSdbug};};	
...
if(_dbug)then{player sideChat format["Armoured:%1 - r%2",_counter,_dGrps];0=[_mkr,_counter,"Armour",(getPos leader(_dGroup select 2))]call EOSdbug};};
...
if(_dbug)then{player sideChat format["Static:%1",_counter];0=[_mkr,_counter,"Static",(getPos leader(_eGroup select 2))]call EOSdbug};};	
...
if(_dbug)then{player sideChat format["Chopper:%1",_counter];0=[_mkr,_counter,"Chopper",(getPos leader(_fGroup select 2))]call EOSdbug};};
...
if(!triggerActivated _eosActv || getMarkerColor _mkr=="colorBlack")exitWith{
	if(_dbug)then{if(!(getMarkerColor _mkr=="colorBlack"))then{hint "Restarting Zone AND deleting units";}else{hint "EOS zone deactivated";};};		
...
if(_dbug)then{player sideChat format["ID:c%1",_cGrps];};};
...
if(_dbug)then{player sideChat format["ID:c%1",_dGrps];};};

//CACHE PATROL INFANTRY					
if(!isNil "_bGrp")then{
_n=0;					
{_n=_n+1;_units={alive _x}count units _x;_cacheGrp=format["PA%1",_n];
if(_dbug) then{player sideChat format["ID:%1,cache - %2",_cacheGrp,_units];};
...
_EOSact=false;
if(_dbug)then{hint "Zone Cached";};};
	...
	while{triggerActivated _EOSactv && !(getMarkerColor _mkr=="colorBlack")}do{
		...
		_mkr setMarkerColor badColor;_mkr setMarkerAlpha _mAH;
			if(_dbug)then{hint "Zone Lost";};}else{_mkr setMarkerColor winColor;_mkr setMarkerAlpha _mAN;
				if(_dbug)then{hint "Zone Captured";};};
sleep 1;};
//player LEFT ZONE				
_EOSact=false;		
};sleep .5;};
...
if(!(getMarkerColor _mkr=="colorBlack"))then{	
null=[_mkr,[_aGrps,_aSize],[_bGrps,_bSize],[_cGrps,_cSize],[_dGrps,_eGrps,_fGrps,_fSize],_setup,true]execVM "eos\core\EOScore.sqf";
