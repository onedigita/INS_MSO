...
private "_noMove";
_noMove=if(count _this>5)then{_this select 5}else{false};
...
if _noMove then{doStop _x;};
if _stance then{_x setUnitPos "UP";};
};};}forEach _men;};

//Hide
if(count _this>7)then{
if(isNil "SHKhousePos_init")then{
SHKhousePos={
private["_act","_arr"];
_act=_this select 0;
_arr=_this select 1;
};
"SHKhousePos_EH" addPublicVariableEventHandler{
private["_act","_arr"];
_arr=_this select 1;
_act=_arr select 0;
_arr=_arr select 1;
//[_act,_arr]call SHKhousePos;
};
SHKhousePos_init=true;};
...
SHKhousePos_EH=[true,_hidden];
publicVariable "SHKhousePos_EH";

waitUntil{(call compile _condition)};
SHKhousePos_EH=[false,_hidden];
publicVariable "SHKhousePos_EH";};};