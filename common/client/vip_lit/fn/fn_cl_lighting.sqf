...
for "_i" from 0 to(count _reflectors-1)do{
		...
		_lampLight=[1-((_playerPos distance _startPos)/_fullDistance),_colour];}else{_endVectors=[];
for "_i" from 0 to(count _reflectors-1)do{
...
if(count _fireLight>0)exitWith{};}forEach _allFires;};
...
if(count _flashLight>1)exitWith{};}forEach _allFlashlights;};
