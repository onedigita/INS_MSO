...
if(count _mapSubs>0)then{_scriptMapSubs=[_respawn,_mapSubs]execVM "common\client\vip_lit\vip_lit_mapSubtractions.sqf";waitUntil{scriptDone _scriptMapSubs}};
if(count _mapAdds>0)then{_scriptMapAdds=[_respawn,_mapAdds]execVM "common\client\vip_lit\vip_lit_mapAdditions.sqf";waitUntil{scriptDone _scriptMapAdds}};
if(count _nvg>0)then{[_respawn,_nvg]call compile preProcessFileLineNumbers "common\client\vip_lit\vip_lit_NVG.sqf"};