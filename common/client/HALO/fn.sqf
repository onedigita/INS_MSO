...
if((_this select 1)==Keys)then{
	...
	{deleteVehicle _x;}forEach _cut;
	player addBackpack "B_Parachute";
	deleteVehicle(player getVariable "frontpack");player setVariable["frontpack",nil,true];
	player setVariable["key",true];player setVariable["cutaway",false];
	CutRope=(findDisplay 46)displayRemoveEventHandler["KeyDown",CutRope];};
	_r=true;
	};
_r;
};
...
_gear=[_headgear,_back_pack,_back_pack_items,_back_pack_weap,_back_pack_maga];
_gear};
setLoadout={
