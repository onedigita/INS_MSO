...
_x setRepairCargo 0;
_x setVariable["zlt_repair_cargo",1,true];};}forEach vehicles;
_first=false;
sleep 26.1;};};};
...
if(!isNil{_dmg})then{
	if( _dmg > 0.64 )then{
		...
		_veh setHitPointDamage[_x,0.64];}else{
...
diag_log str["REPAIR ",_x,_cdmg];
	if(_cdmg > 0.64)exitWith{_rprTime=_rprTime + (if(_x in zlt_hardRepairParts)then{
DEFAULT_FIELDREPAIR_EACH_HARDPART_TIME}else{DEFAULT_FIELDREPAIR_EACH_PART_TIME});};};}forEach zlt_repair_hps;
_rprTime;};
...
if(isNil{_veh})exitWith{};
if(zlt_mutexAction)exitWith{STR_ANOTHER_ACTION call zlt_fnc_notify;};
if(!alive player or(player distance _veh) > 7 or (vehicle player != player)or speed _veh > 3)exitWith{STR_REPAIR_CONDITIONS call zlt_fnc_notify;};
...
_length=_maxlength;
_cycle=0;
while{alive player &&(player distance _veh) < 7 &&(vehicle player==player)&& speed _veh < 3 && !_repairFinished && zlt_mutexAction &&(_cycle < 3 or(["medic",animationState player]call BIS_fnc_inString))}do{
...
_cycle=_cycle + 1;};
	...
	_veh setVariable["zlt_longrepair_times",(_veh getVariable["zlt_longrepair_times",0])+1,true];}else{
...
_veh=cursorTarget;
	...
	if(!alive player or vehicle player==player or speed _veh > 3 or _veh distance _truck > 15 )exitWith{STR_REPAIR_CONDITIONS call zlt_fnc_notify;};
...
_res=_veh distance _truck <= 15 &&{player==(driver _truck)}&&{(_veh isKindOf "LandVehicle" or _veh isKindOf "Ship" or _veh isKindOf "Air")}&&{alive _truck}&&{alive player}&&{alive _veh}&&{!zlt_mutexAction}&&{speed _veh <= 3}&&{(damage _veh != 0)};
_res;};
...
player addAction["<t color='#ff0000'>"+STR_SERIOUS_REPAIR+ "</t>",zlt_fnc_heavyRepair,[],-1,false,true,'','_truck=(vehicle player);_truck getVariable["zlt_repair_cargo",-1] != -1 &&{alive cursorTarget}&&{_truck distance cursorTarget <= 15}&&{(cursorTarget isKindOf "LandVehicle" or cursorTarget isKindOf "Ship" or cursorTarget isKindOf "Air")}&&{!zlt_mutexAction}&&{speed cursorTarget <= 3}&&{(damage cursorTarget != 0)}'];
player addAction["<t color='#FF9900'>"+STR_PUSH_APC+"</t>",zlt_pushapc,[],5,false,true,"","canMove(vehicle player)&&((vehicle player)isKindOf 'Wheeled_APC_F')&& player==driver(vehicle player)&& surfaceIsWater getPos(vehicle player)&& abs(speed(vehicle player)) < 3 && !zlt_mutexAction"];
};
