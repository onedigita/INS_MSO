...
BRS_hideRadius=5;
if(isNil "hideplayer")then{HINT "UNABLE TO LOAD BRS: Make sure you have an object named ""hideplayer"" on the map";}else{
//IF IS ADDACTION
...
_dbug=if(count _BRSarray>4)then{_BRSarray select 4}else{FALSE};
if(_dbug)then{_evType="fired";}else{_evType="killed";};
...
//SCRIPT. IF BRS PREVIOUSLY SET, REMOVE IT AND APPLY NEW BRS
		if(!isNil "brs_EVHandle")then{player removeEventHandler[_evType,brs_EVHandle];};
