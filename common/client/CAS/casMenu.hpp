class casMenu
{
	...
	class Checkbox : JWC_CheckBox
	{
		...
		rows = 1;
		columns = 3;
		strings[] = {"JDAM","CBU(AP)","NONE"};
		checked_strings[] = {"JDAM","CBU(AP)","NONE"};
       	onCheckBoxesSelChanged = "if(_this select 1==0)then{casType='JDAM'};if(_this select 1==1)then{casType='CBU'};if(_this select 1==2)then{casType='NONE'}";
	};
