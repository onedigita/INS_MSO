...
"tcb_ais_in_agony" addPublicVariableEventHandler{
	...
	if(playerSide==_side)then{
		...
		_unit playActionNow "agonyStart";
			if(local player)then{_unit groupChat format["%1 has been incapacitated and needs help!",name _unit]};
...
if(_unit==leader group _unit)then{_unit setVariable["tcb_ais_leader",true];}else{
_unit setVariable["tcb_ais_leader",false];};
...
if(tcb_ais_dead_dialog)then{
	...
	_num=getNumber(missionConfigFile/"respawndelay");
		if(_num!=0)then{
		missionNamespace setVariable["tcb_ais_respawndelay",_num];};}else{
...
if(_unit==player)then{_unit addEventHandler["killed",{_this spawn tcb_fnc_deadcam}];};};