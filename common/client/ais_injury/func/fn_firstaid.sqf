...
if(!isPlayer _healer &&{_healer distance _injuredperson>6})then{
	...
	_timenow=time;
WaitUntil{
_healer distance _injuredperson<=4 ||{!alive _injuredperson}||{!(_injuredperson getVariable "tcb_ais_agony")}||{!alive _healer}||{_healer getVariable "tcb_ais_agony"}||{_timenow+120<time}};};
...
if(!_ableToHeal)exitWith{
	if(tcb_ais_medical_education>=2)then{
		if(isPlayer _healer)then{[format["Only medics are trained for this procedure!" call tcb_ais_font],0,0.035*safezoneH+safezoneY,5,0.3]spawn BIS_fnc_dynamicText};}else{
	...
	if(!isPlayer _healer)then{
	_healer stop false;
	_healer enableAI "MOVE";_healer enableAI "TARGET";_healer enableAI "AUTOTARGET";_healer enableAI "ANIM";};
};
...
if(!isPlayer _healer)then{
_healer stop true;
_healer disableAI "MOVE";_healer disableAI "TARGET";_healer disableAI "AUTOTARGET";_healer disableAI "ANIM";};
if(isPlayer _healer)then{
_animChangeEVH=_healer addEventhandler["AnimChanged",{private["_anim","_healer"];
_healer=_this select 0;
_anim=_this select 1;
	if(primaryWeapon _healer!="")then{
		if(time>=tcb_animDelay)then{tcb_healerStopped=true};}else{
			if(_anim in["amovpknlmstpsnonwnondnon","amovpknlmstpsraswlnrdnon"])then{
			_healer playAction "medicStart";}else{
...
if(isPlayer _healer)then{[_healer,_injuredperson]call tcb_fnc_medicEquipment;};
...
sleep 1;
while{
	time-_time<_damage&&{alive _healer}&&{alive _injuredperson}&&{(_healer distance _injuredperson)<2}&&{!(_healer getVariable "tcb_ais_agony")}&&{!tcb_healerStopped}}do{
sleep 0.5;
...
detach _healer;detach _injuredperson;
...
if(time-_time>_damage)then{
	...
	_injuredperson setDamage _old_damage;}else{
