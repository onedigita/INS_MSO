_unit=_this;
//Remove first aid and Medikits to prevent duplicates, this function uses some sort of Mutex/Semaphore on the unit
...
{if(_x=="FirstAidKit")then{_numFakUniform=_numFakUniform+1;_unit removeItemFromUniform "FirstAidKit";};}forEach _items;

_items=vestItems _unit;
{if(_x=="FirstAidKit")then{_numFaksVest=_numFaksVest+1;_unit removeItemFromVest "FirstAidKit";};}forEach _items;
...
{if(_x=="FirstAidKit")then{_numFaksBackpack=_numFaksBackpack+1;_unit removeItemFromBackpack "FirstAidKit";};
if(_x=="Medikit")then{_numMedi=_numMedi+1;_unit removeItemFromBackpack "Medikit";};}forEach _items;
