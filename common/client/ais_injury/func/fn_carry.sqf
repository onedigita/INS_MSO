...
detach _dragger;detach _injuredperson;
...
if(!alive _injuredperson ||{!alive _dragger}||{(_dragger getVariable "tcb_ais_agony")})then{
	...
	_injuredperson playActionNow "agonyStart";}else{
...
if(alive _injuredperson)then{_injuredperson playActionNow "agonyStart";}else{
...
if(alive _dragger &&{!(_dragger getVariable "tcb_ais_agony")})then{_dragger playMoveNow "amovpknlmstpsraswrfldnon";};