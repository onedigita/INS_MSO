...
waitUntil{
!alive _helper ||{!alive _injuredperson}||{(_helper distance _injuredperson)>2}||{_helper getVariable "tcb_ais_agony"}||{tcb_helperStopped}||{!isNull(_injuredperson getVariable "healer")}};

_helper removeEventHandler["AnimChanged",_animChangeEVH];
detach _helper;detach _injuredperson;

if(alive _helper &&{!(_helper getVariable "tcb_ais_agony")})then{_helper playMove "AinvPknlMstpSlayWrflDnon_AmovPknlMstpSrasWrflDnon";};
