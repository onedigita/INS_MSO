...
civs_FindSpawnPosition = {
	...
	_playerPositions = call civs_GetAllPlayersPositions;
	...
	while { count _playerBuildings > 0 && !_positionFound && _tries < 10 } do {
		...
		//_buildingPosCount = [_building] call civs_CountPositionsInBuilding;
		...
		if (_buildingPosCount > 0) then {
			...
			if (!_tooClose) then {
				if (!([getPos _building, _blackListMarkers] call civs_PositionInsideBlackMarker)) then {
...
civs_PositionInsideBlackMarker={
...
{if([_pos,_x]call civs_PositionIsInsideMarker)then{_isInsideMarker=true;};}forEach _blackListMarkers;
_isInsideMarker
};

civs_FindDestinationPosition={
...
// Pick a building
_buildings = nearestObjects [_unitPos, ["House_EP1"], _maxSpawnDistance];
...
_positionFound = true;};};};}else{
	...
	while { !_positionFound && _tries < 10 } do {
		...
		if (!isOnRoad _pos && !surfaceIsWater _pos && !([_pos, _blackListMarkers] call civs_PositionInsideBlackMarker)) then {
			_foundPosition = _pos;
			_positionFound = true;
		};
	};
...
civs_GetPlayerBuildings = {
	...
	_playerBuildings = [];
	_allPlayerPositions = call civs_GetAllPlayersPositions;
	
	{
		_buildings = nearestObjects [_x, ["House_EP1"], _maxSpawnDistance];
		sleep 0.01;
	...
	{
		_buildingPosCount = [_x] call civs_CountPositionsInBuilding;
		
		if (_buildingPosCount > 0) then {
			if (!([getPos _x, _blackListMarkers] call civs_PositionInsideBlackMarker)) then {
...
//	{
//		[str getPos (_x select 0), getPos (_x select 0), "mil_dot", "ColorYellow", ""] call civs_SetDebugMarkerAllClients;
...
_unitClasses=[_this,"UNIT_CLASSES",["C_man_1"]]call civs_GetParamValue;
_unitsPerBuilding=[_this,"UNITS_PER_BUILDING",0.1]call civs_GetParamValue;
_maxGroupsCount=[_this,"MAX_GROUPS_COUNT",12]call civs_GetParamValue;
_minSpawnDistance=[_this,"MIN_SPAWN_DISTANCE",300]call civs_GetParamValue;
_maxSpawnDistance=[_this,"MAX_SPAWN_DISTANCE",700]call civs_GetParamValue;
_blackListMarkers=[_this,"BLACKLIST_MARKERS",["BL_Mkr0","BL_Mkr1"]]call civs_GetParamValue;
_hideBlacklistMarkers=[_this,"HIDE_BLACKLIST_MARKERS",true]call civs_GetParamValue;
_fnc_OnSpawnCallback=[_this,"ON_UNIT_SPAWNED_CALLBACK",{}]call civs_GetParamValue;
_fnc_OnRemoveCallback=[_this,"ON_UNIT_REMOVE_CALLBACK",{true}]call civs_GetParamValue;
_debug=[_this,"DEBUG",false]call civs_GetParamValue;
if(_hideBlacklistMarkers)then{
{_x setMarkerAlpha 0;}forEach _blackListMarkers;};

_side=civs_SIDE;
_minSkill=civs_MINSKILL;
_maxSkill=civs_MAXSKILL;
...
_pos=[_minSpawnDistance,_playerBuildings,_blackListMarkers]call civs_findSpawnPosition;
_unit=objNull;
...
[_unit]execVM "eos\fn\randOP4.sqf";if(random 1<0.1)then{null=[_unit,"",100,25,west,1,0.1]execVM "eos\fn\civGun.sqf";};

civs_INSTANCE_NO = civs_INSTANCE_NO + 1;
_unit setVehicleVarName "ENGIMA_CIVILIAN_UNIT_" + str civs_INSTANCE_NO;
doStop _unit;_unit setPos _pos;[_unit]spawn _fnc_OnSpawnCallback;};
_unit};
	...
	while{true}do{
		...
		_allPlayerPositions = call civs_GetAllPlayersPositions;
		_playerBuildings = [_allPlayerPositions, _maxSpawnDistance, _blackListMarkers] call civs_GetPlayerBuildings;
		...
		if (count _civilianItems < _unitsCount) then {
			...
			if (!isNull _unit) then {
				...
				_civilianItems pushBack [_unit, "CITIZEN", [], getPos _unit, false, time, random 1 < civs_RUNNINGCHANCE];
			};
		...
		_civilianItemsTemp = [];
		{
			...
			if (_tooClose) then {
				...
				if (!_removeUnit) then {
					...
					[vehicleVarName _civilian] call civs_DeleteDebugMarkerAllClients;
		...
		private["_destPos"];
			...
			if(!_isMoving && time > _nextActionTime)then{
				...
				if(count _destPos>0)then{
					...
					_isMoving=true;
					_isRunning=random 1 < civs_RUNNINGCHANCE;
			...
			if(_isRunning)then{_unit setSpeedMode "NORMAL";}else{_unit setSpeedMode "LIMITED";};
			_x set[3,getPos _unit];
			if(_debug)then{[vehicleVarName _unit,getPos _unit,"mil_dot","ColorWhite","Civ"]call civs_SetDebugMarkerAllClients;};
		}forEach _civilianItems;
		sleep 3;
	};
};
